...
def hi():
    ...
    # capture webcam
    video_capture = cv2.VideoCapture(webcam_id, cv2.CAP_DSHOW)
    time.sleep(1)
    ...
    while True:
        ...
        deep_fake = process_image(source_image,previous,curr_face, generator, kp_detector, relative)
                         
        deep_fake = cv2.cvtColor(deep_fake, cv2.COLOR_RGB2BGR) 
         
        ka=array_to_img(deep_fake)
        ka=np.asarray(ka)

        ret, ya = cv2.imencode('.png', ka)
        fa = ya.tobytes()
        ga = curr_face.tobytes()
        yield (b'--frame\r\n'
...
def process_image(source_image,base,current, generator,kp_detector,relative):
...
def video_feed():
    return Response(hi(), mimetype='multipart/x-mixed-replace; boundary=frame')
